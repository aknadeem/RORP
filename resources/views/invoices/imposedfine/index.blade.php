			...
			<div class="kt-portlet__body">
				...
				<table class="table table-striped table-hover table-checkable" id="kt_table_1">
					...
					<tbody>
						...
						$label_color = 'danger';
						@endphp
						@forelse ($impose_fines as $key=>$impose)
						...
						<tr>
							...
							<td>
								<span style="width: 100px;"><span
										class="btn btn-bold btn-sm btn-font-md  btn-label-success"> {{
										$impose->fine_date->format('d M, Y') }} </span></span>
							</td>
							<td>
								<span style="width: 100px;"><span
										class="btn btn-bold btn-sm btn-font-md  btn-label-danger"> {{
										$impose->due_date->format('d M, Y') }} </span></span>
							...
							<td>
								@if($impose->fine_status == 'pending')
								@can('add-payment-fine-penalties')
								<button imposed-id="{{$impose->id ?? 0}}" fine-id="{{$impose->fine_id ?? 0}}"
									user_id="{{ $impose->user_id ?? 0}}"
									invoice-price="{{$impose->fine->fine_amount ?? 0}}"
									UserName="{{$impose->user->name ?? ''}}"
								...
								@else
								<span style="width: 100px;"><span
										class="btn btn-bold btn-sm btn-label-{{$label_color}}"> <i
											class="fa fa-credit-card"></i> {{ ucfirst($impose->fine_status ?? '')}}
									</span></span>
								@endcan
								@else
								<span style="width: 100px;"><span
							...
							<td>
								<a href="{{ route('imposedfine.show', $impose->id) }}" data-container="body"
...
@section('modal-popup')
@include('_partial.fine_pay_view_modal', ['impose_fines'=> $impose_fines])
@endsection
