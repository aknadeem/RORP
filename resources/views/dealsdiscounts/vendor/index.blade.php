...
<div class="kt-content kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor" id="kt_content">                   
	...
	<div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
		...
		$vendors = $vendors->where('society_id',$search_society_id);
		}else{
		$vendors = $vendors;
		}
		@endphp
					...
					<div class="kt-portlet__head-wrapper">
						<div class="kt-portlet__head-actions">
							@can('create-deals-and-discounts')
								<a href="{{ route('vendors.create') }}" class="btn btn-brand btn-sm btn-elevate btn-icon-sm" title="Create Vendor"><i class="fa fa-plus mb-1"></i>{{ __('Create')}}</a>
							@endcan
						</div>
					</div>
				</div>
				...
				<table class="table table-striped table-hover table-checkable" id="kt_table_1">
					...
					<tbody>
						@forelse ($vendors as $key=>$vendor)
							<tr>
							 	<td>{{ ++$key }}</td>
	                            <td>{{ __($vendor->title) }}</td>
	                            <td> <b> {{$vendor->society->name }} </b> </td>
	                            <td>
	                            	@can('update-deals-and-discounts')
										<a href="{{route('vendors.edit', $vendor->id)}}" class="text-warning"> <i class="fa fa-edit fa-lg" title="Edit Vendor"></i> </a> &nbsp;
									@endcan
									@can('delete-deals-and-discounts')
										<a href="{{route('vendors.destroy', $vendor->id)}}" class="text-danger delete-confirm" del_title="Vendor {{$vendor->title}}"><i class="fa fa-trash-alt fa-lg" title="{{ __('Delete Vendor') }}"></i></a>
									@endcan
								</td>                       
							</tr>
                        @empty
