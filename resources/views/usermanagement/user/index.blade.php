				...
				<div class="kt-portlet__head-toolbar">
					<div class="kt-portlet__head-wrapper">
						<div class="kt-portlet__head-actions">
							...
							<a href="{{ route('users.create') }}" class="btn btn-brand btn-elevate btn-sm btn-icon-sm"
								title="Create User">
								<i class="fa fa-plus mb-1"></i>Create </a>
							@endcan

							@can('view-resident-management')
							<a href="{{ route('residentaccounts') }}"
			...
			<div class="kt-portlet__body">
				...
				<table class="table table-striped table-hover table-checkable" id="kt_table_1">
					...
					<tbody>
						...
						<tr>
							<td>{{++$key}}</td>
							<td> <a href="{{ ($user->user_level_id < 6) ? route('users.show', $user->id) : $url }}"
									title="{{ ($user->user_level_id < 6) ? $htitle : $title }}"> {{$user->name}} </a>
							</td>
							<td> <b> {{$user->society->name ?? '' }} </b> </td>
							<td>
								@if (count($user->permissions) > 0)
								<a href="{{ route('user-permissions', $user->id) }}" title="view Permissions"
								...
								<a href="{{ route('edituserpermissions', $user->id) }}" title="Add Permissions"
							...
							<td>
								...
								<a href="{{ route('user.toggleStatus',$user->id) }}"
									...
									<label class="kt-switch kt-switch--outline kt-switch--icon kt-switch--success">
										...
										<span {{ $user->is_active == 1 ? 'title=Deactive' : 'title=Active' }}
											class="slider {{ $user->is_active == 1 ? '' : '' }}"></span>
									</label>
								</a>
								@else
							...
							<td>
								@can('update-user-management')
								<a href="{{ route('users.edit', $user->id) }}" class="text-warning"> <i
