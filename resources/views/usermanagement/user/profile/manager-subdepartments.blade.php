...
<div class="kt-content kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor" id="kt_content">              
    ...
    <div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
		...
		<div class="kt-grid kt-grid--desktop kt-grid--ver kt-grid--ver-desktop kt-app">
                        ...
                        <div class="kt-portlet">
                            <div class="kt-portlet__head">
                                <div class="kt-portlet__head-label">
                                    <h3 class="kt-portlet__head-title"> Manager SubDepartments
                                    </h3>
                                </div>
                                <div class="kt-portlet__head-toolbar">
									 <a class="btn btn-label-brand btn-bold btn-sm AdminSocietyModal" title="Attach Subdepartment">
										<i class="fa fa-plus"></i> Subdepartment
									</a>
								</div>
                            </div>
										...
										<table class="table table-striped table-hover table-checkable" id="kt_table_1">
											<thead>
												<tr>
													...
													<th>{{ __('SubDepartment Name')}}</th>
													<th class="text-center">{{ __('Action')}} </th>
												</tr>
											</thead>
											<tbody>
												@forelse ($user->subdepartments as $key=>$sub_dep)
													<tr>
														<td>{{++$key}}</td>
														<td>{{$sub_dep->subdepartment->department->name}}</td>
														<td>{{$sub_dep->subdepartment->name}}</td>
														<td class="text-center">
															@if (auth()->user()->user_level_id < 4)
																@can('delete-user-management')
																    <a href="{{route('deattach.subdepartment', $sub_dep->sub_department_id)}}" class="text-danger confirm-status" msg="Are You Sure To <b class='text-danger'>DeAttach</b> <b class='text-success'>{{$sub_dep->subdepartment->name}}</b> From <b>{{$user->name}}</b>"><i class="fa fa-trash-alt fa-lg" title="DeAttach"></i></a>
																@endcan
															@endif
														</td>
													</tr>
												@empty
												<tr>
													<td colspan="3"> No Subdepartment Found </td>
												</tr>
												@endforelse
											</tbody>
										</table>
            ...
            {{-- Pass User id TO Deattach Subdepartment From Manager --}}
...
@endsection
@section('modal-popup')
    <div class="modal fade" id="AdminSocietyModal" tabindex="-1" role="dialog" aria-labelledby="PermissonsModal" aria-hidden="true">
    	<div class="modal-dialog" role="document">
    		<div class="modal-content">
    			<div class="modal-header">
    				<h5 class="modal-title">{{ __('Attach Subdepartment')}}</h5>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="form-group validated col-sm-12">
                                ...
                                <input type="text" class="form-control" name="manager_id" readonly id="manager_id" disabled  value="{{$user->name}}">
                                @error('manager_id')
                        ...
                        <div class="row">
                            <div class="form-group validated col-sm-12">
                                ...
                                <select class="form-control kt-selectpicker @error('subdepartment_id') is-invalid @enderror" data-live-search="true"  name="subdepartment_id" >
                                    <option selected disabled> <b> {{ __('Select Subdepartment')}} </b> </option>

                                    @forelse($subdepartments as $sb_dep)
                                        <option value="{{$sb_dep->id}}">{{ $sb_dep->name }}</option>    
                                    @empty
                                        <option disabled> No subdepartment Found </option>
                                    @endforelse
...
@section('scripts')
	...
	<script>
		$('.AdminSocietyModal').click(function () {
			$('#AdminSocietyModal').modal('show');
		});
	</script>
@endsection