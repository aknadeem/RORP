...
<div class="kt-content kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor" id="kt_content">              
    ...
    <div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
		...
		<div class="kt-grid kt-grid--desktop kt-grid--ver kt-grid--ver-desktop kt-app">
                        ...
                        <div class="kt-portlet">
                            <div class="kt-portlet__head">
                                <div class="kt-portlet__head-label">
                                    <h3 class="kt-portlet__head-title"> Admin Societies
                                    </h3>
                                </div>
                                <div class="kt-portlet__head-toolbar">
                                	@if (auth()->user()->user_level_id == 1)
										<a class="btn btn-label-brand btn-bold btn-sm AdminSocietyModal">
											<i class="fa fa-plus"></i> Add Societies
										</a>
									@endif
								</div>
                            </div>
                                ...
                                <div class="kt-section kt-section--first">
                                    <div class="kt-section__body">
										...
										<table class="table table-striped table-hover table-checkable" id="kt_table_1">
											...
											<tbody>
												@forelse ($user->societies as $key=>$society)
													<tr>
														<td>{{++$key}}</td>
														<td>{{$society->code}}</td>
														<td>{{$society->name}}</td>
														...
														@can('delete-user-management')
															@if (auth()->user()->user_level_id < 2)
																<td class="text-center">
																    <a href="{{route('deattach.society', $society->id)}}" class="text-danger confirm-status" msg="Are You Sure To <b class='text-danger'>DeAttach</b> <b class='text-success'>{{$society->name}}</b> From <b>{{$user->name}}</b>"><i class="fa fa-trash-alt fa-lg" title="DeAttach"></i></a>
																</td>
															@endif
														@endcan
													</tr>
												@empty
												<tr>
													<td colspan="4"> No Society Found </td>
												</tr>
												@endforelse
											</tbody>
										</table>
            ...
            {{-- Pass User id To Deattach Society From Admin --}}
    	...
    	<div class="modal-dialog" role="document">
    		<div class="modal-content">
    			<div class="modal-header">
    				<h5 class="modal-title">{{ __('Add Societies')}}</h5>
    				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
    				</button>
    			</div>
    			<form action="{{ route('admin.addsocieties')}}" method="POST">
    				@csrf
					<div class="modal-body">
						<div class="row">
							<div class="form-group validated col-sm-12">
	                            ...
	                            <input type="hidden" name="user_id" value="{{$user->id}}">
	                            <input type="text" readonly disabled name="user_name" class="form-control" value="{{$user->name}} [ {{$user->level_slug}} ]">
	                        </div>
	                        <div class="form-group validated col-sm-12">
	                            <label class="form-control-label"><b>{{ __('Select Society:*') }}</b></label>
	                            <select class="form-control kt-selectpicker" name="society_id[]" data-live-search="true" multiple required>
