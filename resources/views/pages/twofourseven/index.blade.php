...
<div class="kt-content kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor" id="kt_content">                   
    ...
    <div class="kt-subheader kt-grid__item" id="kt_subheader">
        <div class="kt-container kt-container--fluid">
            <div class="kt-subheader__main">
                ...
                <a href="{{ route('socialmedia.index') }}"><span class="kt-subheader__desc">{{ __('24/7')}}</span></a>
            </div>
        </div>
    </div>
	...
	<div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
        ...
        @endphp
	     @include('_partial.search_with_societies', ['society_id' => $society_id,'societies'=> $societies, 'filter_url' => 'twofours_filter'])
		<div class="kt-portlet kt-portlet--mobile">
					...
					<div class="kt-portlet__head-wrapper">
						<div class="kt-portlet__head-actions">
							@can('create-pages')
								...
								&nbsp;
								<a href="{{ route('twofour.create') }}" class="btn btn-brand btn-sm btn-elevate btn-icon-sm" title="Create 24/7"><i class="fa fa-plus mb-1"></i>{{ __('Create')}}</a>
							@endcan
						</div>
					</div>
			...
			<div class="kt-portlet__body">
				...
				<table class="table table-striped table-hover" id="kt_table_1">
					...
					<tbody>
						@forelse ($two4sevens as $two4seven)
							<tr>
							 	<td>{{ $two4seven->id }}</td>
	                            <td> {{$two4seven->title}} </td>
	                            <td>
	                            	{{-- @if ($two4seven->societies->count() > 0) --}}
                                        <button type="button" class="btn btn-bold btn-label-brand btn-sm twoFourSevenSocieties" title="View Societies" two_four_title="{{ __($two4seven->title) }}" two_four_id="{{$two4seven->id}}"><i class="fa fa-eye mb-1"></i>view </button>
                                    {{-- @endif --}}
	                            </td>
	                            <td> {!! Str::limit(preg_replace("/<img[^>]+\>/i", "(image) ", $two4seven->description), 80) !!} </td>
	                            <td>
	                            	<a href="{{ route('twofour.show', $two4seven->id) }}"> <i class="fa fa-eye fa-lg" title="View Detail"></i> </a>
	                            	&nbsp;
	                            	@can('update-pages')
										<a href="{{route('twofour.edit', $two4seven->id)}}" class="text-warning"> <i class="fa fa-edit fa-lg" title="Edit 24/7"></i> </a> &nbsp;
									@endcan
									@can('delete-pages')
										<a href="{{route('twofour.destroy', $two4seven->id)}}" class="text-danger delete-confirm" del_title="24/7 {{$two4seven->title}}"><i class="fa fa-trash-alt fa-lg" title="{{ __('Delete 24/7') }}"></i></a>
									@endcan
								</td>                       
							</tr>
                        @empty
...
@section('modal-popup')
	...
	@include('_partial.upload_pages_attachment', ['upload_url'=> route('uploadtwofour.attachment')])

	@include('_partial.view_attachments', ['modal_title' => 'TwoFour','attachments'=> $attachments, 'delete_url' => 'deletetwofour_attachment'])
@endsection
@section('top-styles')
...
@section('scripts')
    ...
    <script>
    	$(".twoFourSevenSocieties").click(function(event) {
		    ...
		    var two4sevens = <?php echo json_encode($two4sevens); ?>;
