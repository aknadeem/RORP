    ...
    <div class="kt-subheader kt-grid__item" id="kt_subheader">
        <div class="kt-container kt-container--fluid">
            <div class="kt-subheader__main">
                <h3 class="kt-subheader__title">{{ __('24/7')}}</h3>
                <span class="kt-subheader__separator kt-subheader__separator--v"></span>

                <a href="{{ route('twofour.index') }}"><span class="kt-subheader__desc">{{ __('24/7')}}</span></a>
            </div>
        </div>
    </div>
    <!-- end:: Content Head -->
    ...
    <div class="row">
        <div class="col-lg-12">
            ...
            <div class="kt-portlet">
                <div class="kt-portlet__head">
                    <div class="kt-portlet__head-label">
                        <h3 class="kt-portlet__head-title">
                            {{ __(($twoFourSeven->id > 0 ? "Edit" : "Create").' 24/7') }}
                        </h3>
                    </div>
                    <div class="kt-subheader__toolbar">
                        <div class="kt-subheader__wrapper">
                            <a href="{{ route('twofour.index') }}" class="btn btn-brand btn-bold btn-sm kt-margin-l-10 mt-3">
                               {{ __('24/7')}}
                            </a>
                        </div>
                    </div>
                ...
                <!--begin::Form-->
                <form class="kt-form" action="{{ ($twoFourSeven->id) ? route('twofour.update', $twoFourSeven->id ) : route('twofour.store') }}" method="post">
                    @csrf
                    @if($twoFourSeven->id)
                        @method('PUT')
                    @endif
                        ...
                        <div class="kt-section kt-section--first mb-0">
                            <div class="row">
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="text" class="form-control" name="title" value="{{ $twoFourSeven->title ?? old('title') }}" placeholder="Enter Title" autofocus />
                                </div>
                                <div class="form-group validated col-sm-6">
                                    ...
                                    <select class="form-control kt-selectpicker @error('social_type') is-invalid @enderror" name="societies[]" multiple data-live-search="true">
                                            ...
                                            @forelse ($societies as $society)
                                                <option {{ (in_array($society->id, old('societies', [])) || $twoFourSeven->societies->contains($society->id)) ? 'selected' : '' }} value="{{$society->id}}"> {{$society->name}} </option> 
                                            @empty
                                ...
                                <div class="form-group col-md-12">
                                    ...
                                    <textarea class="summernote" name="description" id="kt_summernote_1">{!! $twoFourSeven->description ?? '' !!}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
