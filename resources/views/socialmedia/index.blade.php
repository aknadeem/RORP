					...
					<div class="kt-portlet__head-wrapper">
						<div class="kt-portlet__head-actions">
							@can('create-social-management')
								<a href="{{ route('socialmedia.create') }}" class="btn btn-brand btn-sm btn-elevate btn-icon-sm" title="Create Social Media"><i class="fa fa-plus mb-1"></i>{{ __('Create')}}</a>
							@endcan
						</div>
					</div>
				</div>
					...
					<tbody>
						@forelse ($socialmedias as $media)
    						@if($search_society_id !='all' AND $search_society_id !='')
        						@if($media->societies->where('id',$search_society_id)->count() > 0)
        						<tr>
    							 	<td>{{ $media->id }}</td>
    							 	<td>{{ $media->social_type }}</td>
    							 	 <td> <button type="button" class="btn btn-bold btn-label-brand btn-sm openSocietiesModal" title="View Societies" modal_title="{{ __($media->title) }}" source_id="{{$media->id}}"><i class="fa fa-eye mb-1"></i>view </button> </td>
    							 	<td>{{ $media->title }}</td>
    	                            <td> <a href="{{ $media->link_url }}" target="_blank"> {{ $media->link_url }} </a> </td>
    	                            <td>
    	                            	@can('create-social-management')
    										<a href="{{route('socialmedia.edit', $media->id)}}" class="text-warning"> <i class="fa fa-edit fa-lg" title="Edit Social Media"></i> </a> &nbsp;
    									@endcan
    									
    									@can('delete-social-management')
    										<a href="{{route('socialmedia.destroy', $media->id)}}" class="text-danger delete-confirm" del_title="Social Media {{$media->title}}"><i class="fa fa-trash-alt fa-lg" title="{{ __('Delete Social Media') }}"></i></a>
    									@endcan
    								</td>                       
    							</tr>
    							@endif
...
@section('modal-popup')
	@include('_partial.view_societies_modal', ['data'=> $socialmedias])
@endsection
