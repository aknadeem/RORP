						...
						<div class="kt-widget__content">
							<div class="kt-widget__head">
								<div class="kt-widget__user">
									...
									<span
										class="kt-badge kt-badge--bolder kt-badge kt-badge--inline kt-badge--unified-success p-2">{{
										$user_services->level_slug }}</span>

								</div>
							</div>
							<div class="kt-widget__subhead mb-2">
								<span><i class="fa fa-hands-helping"></i> Total Services: &nbsp; <b>{{
										$user_services->totalServices}}</b> </span>
							</div>
						</div>
					</div>
				</div>
						...
						<div class="kt-portlet__head-toolbar">
							<div class="kt-portlet__head-wrapper">
								<div class="kt-portlet__head-actions">
									@if (Auth::user()->user_level_id < 6) @can('create-invoices') <a
										href="{{ route('minvoice.create', $user_services->id) }}"
										class="btn btn-brand btn-sm  btn-elevate btn-icon-sm" data-toggle="kt-tooltip"
										data-placement="bottom" data-skin="brand" title="Click to create Invoice"><i
											class="fa fa-plus"></i>{{ __('Create Invoice')}}</a>
										@endcan
										@endif
								</div>
							</div>
							...
							<tbody>
								...
								<tr>
									...
									<td class="text-success"> <b> {{
											number_format($user_service->discount_amount,0)}}</b> </td>
									<td class="text-danger"><b> {{number_format($user_service->final_price,0)}}</b>
									</td>
									<td>{{$user_service->start_date->format('Y-m-d') ?? ''}}</td>
									<td>
										@if (Auth::user()->user_level_id < 6) <a
											href="{{ route('userservices.edit',$user_service->id) }}"
											...
											<label
												...
												>
												<span {{ $user_service->status == 1 ? 'title=Deactive' : 'title=Active'
													}} class="slider {{ $user_service->status == 1 ? '' : '' }}"></span>
											</label>
											</a>
											@else
											<h6 class="text-{{ ($user_service->status == 1) ? 'success' :
												'danger' }}"> {{ ($user_service->status == 1) ? 'Active' :
												'Inactive' }} </h6>
											@endif
									</td>
									<td>

										@if (Auth::user()->user_level_id < 6) <span
											service_title="{{$user_service->service->title}}"
											service_package="{{$user_service->package->title}}"
											ex-price="{{$user_service->price}}" sr_id="{{$user_service->id}}"
											ex-discount-percent="{{$user_service->discount_percentage}}"
											ex-discount="{{$user_service->discount_amount}}"
											class="btn btn-primary btn-sm openPaymentModel" title="Add Discount">
					...
					<div class="row">
						<div class="form-group col-lg-12">
							...
							<div class="input-group">
								<input type="text" id="ExServiceTitle" class="form-control" readonly disabled
									placeholder="Service Title" style="border-radius: 3px;">
							</div>
						</div>
						<div class="form-group col-lg-12">
							<label class="form-control-label"><b>{{ __('Package Title:') }}</b></label>
							<div class="input-group">
								<input type="text" id="ExPackageTitle" class="form-control" readonly disabled
									placeholder="Package Title" style="border-radius: 3px;">
							</div>
						</div>
						...
						<div class="form-group col-lg-6">
							...
							<div class="input-group date">
								<input type="number" step="any" min="0" max="" required id="discount_amount"
									name="discount_amount" class="form-control" placeholder="Enter Discount">
								@error('payed_amount')
						...
						<div class="form-group col-lg-6">
							...
							<div class="input-group">
								<input type="number" step="any" min="0" max="100" required id="discount_percent"
									name="discount_percent" class="form-control" placeholder="Discount Amount %">
							</div>
						</div>
						...
						<div class="form-group col-lg-6">
							...
							<div class="input-group">
								<input type="number" readonly disabled step="any" min="0" id="pckgPrice"
						...
						<div class="form-group col-lg-6">
							...
							<div class="input-group">
								<input type="number" step="any" min="0" max="" name="final_amount" id="finalAmount"
									readonly class="form-control" placeholder="Final Amount">
							</div>
