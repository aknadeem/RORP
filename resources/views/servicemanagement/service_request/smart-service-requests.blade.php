				...
				<div class="kt-portlet__head-toolbar">
					<div class="kt-portlet__head-wrapper">
						<div class="kt-portlet__head-actions">
							{{-- <a href="{{ route('requestservice.create') }}" class="btn btn-brand btn-sm btn-elevate
							btn-icon-sm" title="Request Service"><i class="fa fa-plus mb-1"></i>{{ __('Create')}}</a>
							--}}
						</div>
					</div>
				</div>
				...
				<table class="table table-striped table-hover table-checkable kt_table_11">
					...
					<tbody>
						...
						<tr>
							...
							<td>
								<span class="btn btn-{{$service_req->status_color}} btn-sm">{{ $service_req->status
									}}</span>

								@if ($user_level_id < 5 ) @if ($service_req->service->billing_type !='no_billing' AND
									$service_req->package_id > 0)

									@if ($service_req->status != 'approved')
									@if($service_req->status == 'paid' && $account_hod_id == $user_id)
									@else
							...
							<td>
								@can('update-service-management')
								<a href="{{route('requestservice.edit', $service_req->id)}}" data-toggle="kt-tooltip"
								...
								<a href="{{route('requestservice.destroy', $service_req->id)}}"
									...
									del_title="Service Request {{ substr($service_req->title, 0, 20)}}"><i
										...
										title="{{ __('Delete Service Request') }}"></i></a>
								@endcan --}}
							</td>
						</tr>
						@empty
