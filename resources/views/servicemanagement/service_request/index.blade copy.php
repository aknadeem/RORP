			...
			<div class="row alert alert-light alert-elevate" role="alert">
				...
				<div class="form-group validated col-sm-3 col-xs-6">
					...
					<select class="form-control kt-selectpicker" name="search_department_id" data-live-search="true"
						...
						<option @if ($search_department_id==$cp_department->id)
							selected
							@endif value="{{$cp_department->id}}"> {{$cp_department->name}}</option>
						@endforeach
					</select>
				</div>
				<div class="form-group validated col-sm-3 col-xs-6">
					...
					<select class="form-control kt-selectpicker" name="search_subdep_id" data-live-search="true"
						...
						<option @if ($search_subdep_id=='all' ) selected @endif value="all"> All </option>
						@foreach ($departments as $department)
						@foreach ($department->subdepartments as $sub)
						<option @if ($search_subdep_id==$sub->id)
							selected
				...
				<div class="form-group validated col-sm-2 col-xs-6">
					...
					<select class="form-control kt-selectpicker" name="search_status" data-live-search="true">
						...
						<option @if ($search_status=='all' ) selected @endif value="all"> All </option>
						<option @if ($search_status=='open' ) selected @endif value="open"> Open </option>
						<option @if ($search_status=='closed' ) selected @endif value="closed"> Closed </option>
						<option @if ($search_status=='approved' ) selected @endif value="approved"> Approved </option>
						<option @if ($search_status=='in_process' ) selected @endif value="in_process"> In Process
						</option>
						<option @if ($search_status=='in_correct' ) selected @endif value="in_correct"> In Correct
						</option>
					</select>
				</div>
					...
					<tbody>
						...
						<tr>
							...
							<td>
								...
								<a href="{{route('requestservice.show', $service_req->id)}}" data-toggle="kt-tooltip"
									data-placement="bottom" data-skin="brand" title="Click to view Detail Detail">
									&nbsp; {{ $service_req->service->title }}
								</a> &nbsp;
								@else
								{{ $service_req->service->title }}
								@endcan
							</td>
							<td><b> {{$service_req->user->name}}</b></td>

							<td>{{ $service_req->service->billing_type }}</td>
							<td>{{ $service_req->total_price ?? 0 }}</td>
							<td>{{ $service_req->price_include_tax ?? 0 }}</td>
							<td>
								<span class="badge badge-{{$service_req->status_color}}">{{ $service_req->status }}
								</span>

								@can('update-service-management')

								@if ($user_level_id < 5 ) @if ($service_req->service->billing_type !='no_billing'
									AND $service_req->package_id > 0)

									@if ($service_req->status != 'approved')
									<span request-id="{{$service_req->id}}"
										data-pckg-price="{{$service_req->package->price}}"
										request_ispaid="{{$service_req->is_paid}}"
									...
									{{-- <span data-toggle="modal" data-target="#close-re-assign"
										data-target-id="{{$service_req->id}}"
										class="btn btn-sm btn-clean btn-icon btn-icon-md" data-toggle="kt-tooltip"
										data-placement="bottom" data-skin="brand" title="Click to Update">
										<i class="flaticon-edit"></i>
									</span> --}}
									@endif

									@if ($service_req->status != 'completed')
									<span data-toggle="modal" data-target="#todoWorking"
										data-target-id="{{$service_req->id}}" service_status="{{$service_req->status}}"
										class="btn btn-sm btn-clean btn-icon btn-icon-md" data-toggle="kt-tooltip"
										data-placement="bottom" data-skin="brand" title="Click to Update">
										<i class="flaticon-edit"></i>
									</span>
							...
							<td>
								@can('update-service-management')
								<a href="{{route('requestservice.edit', $service_req->id)}}" class="text-warning"> <i
										class="fa fa-edit fa-lg" data-toggle="kt-tooltip" data-placement="bottom"
										data-skin="brand" title="Click to Edit Service Request"></i> </a> &nbsp;
								@endcan
								@can('delete-service-management')
								<a href="{{route('requestservice.destroy', $service_req->id)}}"
									class="text-danger delete-confirm"
									del_title="Service Request {{ substr($service_req->title, 0, 20)}}"><i
										class="fa fa-trash-alt fa-lg" data-toggle="kt-tooltip" data-placement="bottom"
										data-skin="brand" title="Click to Delete Service Request"></i></a>
								@endcan
							</td>
						</tr>
						@empty
