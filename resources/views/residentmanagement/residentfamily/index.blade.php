...
<div class="kt-content kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor" id="kt_content">         
    ...
    <div class="kt-subheader kt-grid__item" id="kt_subheader">
        <div class="kt-container kt-container--fluid">
            <div class="kt-subheader__main">
                ...
                <a href="{{ route('residentdata.index') }}"><span class="kt-subheader__desc">{{ __('Resident Family')}}</span></a>
            </div>
        </div>
    </div>
    <!-- end:: Content Head -->
    ...
    <div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
		...
		$resfamilies = $resfamilies->where('society_id', $search_society_id);
		}else{
		$resfamilies = $resfamilies;
		}
		
		if($search_landlord_id !='all' AND $search_landlord_id !=''){
		$resfamilies = $resfamilies->where('resident_data_id', $search_landlord_id);
		}else{
		$resfamilies = $resfamilies;
		}
		@endphp
			...
			<div class="row alert alert-light alert-elevate" role="alert">
				...
				<div class="form-group validated col-sm-4 col-xs-4">
					...
					<select class="form-control kt-selectpicker" name="search_landlord_id" data-live-search="true">
						...
						@foreach ($resfamilies as $resident)
					...
					<div class="kt-portlet__head-wrapper">
						<div class="kt-portlet__head-actions">
							@can('create-resident-management')
								<a href="{{ route('residentfamily.create') }}" class="btn btn-brand btn-sm btn-elevate btn-icon-sm" title="Create User">
					...
					<tbody>
						@foreach ($resfamilies as $user)
							<tr>
								...
								<td>
									@can('create-resident-management')
	                                	<a href="{{ route('residentfamily.edit', $user->id) }}" class="text-warning"> <i class="fa fa-edit fa-lg" title="Edit Family Member"></i> </a> &nbsp;
	                                @endcan

									@can('delete-resident-management')
									    <a href="{{route('residentfamily.destroy', $user->id)}}" class="text-danger delete-confirm" del_title="Family Member {{$user->name}}"><i class="fa fa-trash-alt fa-lg" title="Delete Family Member"></i></a>
									@endcan
								</td>
							</tr>
						@endforeach
